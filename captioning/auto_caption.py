...
class AutoCaptionPipeline:
    ...
    def caption_image(self, image_path: str,
        ...
        if method in ("blip2", "florence2", "combined"):
            result.natural_description = self.natural_captioner.caption(
        ...
        for item in items:
            ...
            if ext in VIDEO_EXTENSIONS:
                if pipeline_mode == "image":
                    ...
                    if item.caption_text and not self.config.overwrite_existing:
                        results.append(CaptionResult(
                            file_path=item.original_path,
                            combined_caption=item.caption_text))
                ...
                if item.caption_text and not self.config.overwrite_existing:
                    ...
                    if progress_callback:
                        progress_callback(
                            len(results), total_caption_count + len(image_tasks),
                            item.original_path)
                    continue
                ...
                try:
                    ...
                    if use_nl and pil_img is not None:
                        result.natural_description = (
                            self.natural_captioner.caption(
