...
class DatasetManager:
    ...
    def __init__(self, dataset_dir: str, work_dir: Optional[str] = None):
        ...
        if work_dir:
            ...
            dir_hash = hashlib.md5(str(self.dataset_dir.resolve()).encode()).hexdigest()[:12]
    ...
    def scan_directory(self, recursive: bool = True) -> List[DatasetItem]:
        """Scan dataset directory for all supported media files."""
        self.items.clear()
        all_extensions = IMAGE_EXTENSIONS | VIDEO_EXTENSIONS

        pattern = "**/*" if recursive else "*"
        for fpath in sorted(self.dataset_dir.glob(pattern)):
        ...
        for idx, item in enumerate(self.items):
            try:
                if item.media_type in ("image", "animated"):
                    self._convert_image(item, converted_dir, target_format, max_resolution)
            ...
            if progress_callback:
                progress_callback(idx + 1, total, item.original_path)
    ...
    def _convert_image(self, item: DatasetItem, out_dir: Path,
        ...
        if item.media_type == "animated":
            self._extract_animated_frames(item, out_dir, target_format, max_res)
            return
    ...
    def _convert_video(self, item: DatasetItem, out_dir: Path, max_res: int):
        ...
        Uses sequential reading (no seeking) to avoid h264
        'mmco: unref short failure' hangs on compressed streams.
        """
        import cv2

        src = Path(item.original_path)
        # Prefer ffmpeg backend — more tolerant of h264 quirks
        ...
        # Extract ~1 frame per second, max 30 frames — sequential read
        max_frames = 30
        ...
        while extracted < max_frames:
            ...
            if frame_idx % interval == 0:
                fh, fw = frame.shape[:2]
                if max(fw, fh) > max_res:
                    ratio = max_res / max(fw, fh)
                    new_w = (int(fw * ratio) // 8) * 8
                    new_h = (int(fh * ratio) // 8) * 8
                    frame = cv2.resize(frame, (new_w, new_h), interpolation=cv2.INTER_LANCZOS4)

                out_name = f"{src.stem}_{base_hash}_v{extracted:03d}.png"
                out_path = out_dir / out_name
        ...
        cap.release()
        item.converted_path = str(out_dir)
        item.is_valid = extracted > 0
        if not item.is_valid:
            item.error = "No frames extracted"
        else:
            logger.info(f"Extracted {extracted} frames from {src.name} (sequential read)")
            ...
            if converted.is_file():
                ...
                for frame_file in sorted(converted.glob("*.png")):
                    if Path(item.original_path).stem in frame_file.stem:
                        ...
                        # Apply same caption to all frames
